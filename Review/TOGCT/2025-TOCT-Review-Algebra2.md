## 2025-December-04 - Factoring Trinomials With Leading Coefficient not 1 - AC Method & By Grouping - Algebra - 3 Terms : 
https://www.youtube.com/watch?v=4v-EQIxqpMQ
- For the **AC Method** Whenever You are trying to factor by grouping and you can't see a GCF to take out, you can just use '1'
### Factoring $6x^2 + 11x - 10$ using the AC Method

* **1. Identify Coefficients**
    * The trinomial is in the form $ax^2 + bx + c$.
    * $a = 6$
    * $b = 11$
    * $c = -10$

* **2. Calculate the AC Product**
    * $A \cdot C = (6) \cdot (-10) = \mathbf{-60}$

* **3. Find Factors of AC that Sum to B**
    * We need two numbers, $p$ and $q$, such that:
        * $p \cdot q = -60$
        * $p + q = 11$
    * Since the product is negative, one factor is positive and one is negative. Since the sum (11) is positive, the larger factor must be positive.
    * The factors are: **$15$ and $-4$** ($15 + (-4) = 11$ and $15 \cdot (-4) = -60$).

* **4. Rewrite the Middle Term ($11x$)**
    * Replace $11x$ with the two new terms, $15x$ and $-4x$.
    * $6x^2 \mathbf{+ 15x - 4x} - 10$

* **5. Factor by Grouping**
    * Group the first two terms and the last two terms.
    * $(6x^2 + 15x) + (-4x - 10)$
    * Factor out the Greatest Common Factor (GCF) from each group.
        * $\text{GCF}(6x^2, 15x) = 3x \implies 3x(2x + 5)$
        * $\text{GCF}(-4x, -10) = -2 \implies -2(2x + 5)$
    * $3x(2x + 5) - 2(2x + 5)$
...
The factored form of $6x^2 + 11x - 10$ is **$(2x + 5)(3x - 2)$**.
...
### Factoring $3x^3 - 2x^2 - 12x + 8$ by Grouping
...
* **2. Factor out the GCF from Each Group**
    * **First Group ($3x^3 - 2x^2$):** The Greatest Common Factor (GCF) is $x^2$.
        * $x^2(3x - 2)$
    * **Second Group ($-12x + 8$):** The GCF is $-4$. (Factor out the negative to ensure the binomial matches the first group).
        * $-4(3x - 2)$

* **3. Identify the Common Binomial**
    * The expression now is: $x^2(3x - 2) - 4(3x - 2)$
    * The common binomial is **$(3x - 2)$**.

* **4. Factor out the Common Binomial**
    * Factor out $(3x - 2)$ from the entire expression.
    * $(3x - 2)(x^2 - 4)$

* **5. Factor the Remaining Binomial (Difference of Squares)**
    * The factor $(x^2 - 4)$ is a **Difference of Squares** (since $x^2$ is a perfect square and $4 = 2^2$).
    * Recall the formula: $a^2 - b^2 = (a - b)(a + b)$.
    * $x^2 - 4 = \mathbf{(x - 2)(x + 2)}$
...
### **Problem: Solve for x in $\mathbf{9x^2 - 64 = 0}$ by Factoring**
...
Consequently, squaring $i$ removes the radical:
$$\mathbf{i^2 = -1}$$

---

### 2. Complex Numbers in Standard Form
A **complex number** combines a real part and an imaginary part. 
**Standard Form:** $$\mathbf{a + bi}$$

* **$a$**: The **Real Part**
* **$bi$**: The **Imaginary Part** ($b$ is a real number)

> [!INFO] Definitions
> * If $b = 0$, the number is a **purely real number**.
> * If $a = 0$ and $b \neq 0$, it is a **purely imaginary number**.

---

### 3. Simplifying Negative Radicals
To simplify the square root of a negative number, factor out $\sqrt{-1}$ as $i$.
* **Rule:** $\sqrt{-x} = i\sqrt{x}$
* **Example 1:** $\sqrt{-25} = \sqrt{25} \cdot \sqrt{-1} = \mathbf{5i}$
* **Example 2:** $\sqrt{-12} = \sqrt{4 \cdot 3 \cdot -1} = \mathbf{2i\sqrt{3}}$
...
For any complex number $a + bi$, its **complex conjugate** is $\mathbf{a - bi}$.
* **Significance:** When you multiply a complex number by its conjugate, the $i$ terms cancel out, resulting in a **real number**:
  $$(a + bi)(a - bi) = a^2 + b^2$$
...
### **Problem: Solve $\log(x)^{\log(x)} = 49$**

This problem features a logarithm in the exponent. To solve it, we will use substitution to simplify the expression into a standard power equation.

#### **1. Use Substitution**
Let $u = \log(x)$. 
Now we can rewrite the original equation using $u$:
$$u^u = 49$$

#### **2. Solve for $u$**
We need to find a number which, when raised to itself, equals 49.
* Try $1^1 = 1$
* Try $2^2 = 4$
* Try $3^3 = 27$
* Try $4^4 = 256$

Since 49 is between 27 and 256, the answer is not a simple integer. However, we can look at the structure of 49. We know that:
$$49 = 7^2$$

If we rewrite our equation $u^u = 7^2$, we can see a relationship. Notice that $7 = \sqrt{49}$. This suggests we should test if $u$ is related to a value that allows the base and exponent to match.

Wait—let's look at the equation again. If $u = \log(x)$, and we have:
$$\log(x)^{\log(x)} = 7^2$$
There is no clean integer $u$ that satisfies $u^u = 49$. This type of equation ($x^x = c$) usually requires the **Lambert W Function** for an exact analytical solution.

However, if the problem was intended to be **$\log(x^2)$** or if there is a specific property at play, we can approximate. 

#### **3. Numerical Approximation**
If $u^u = 49$:
Using a calculator or trial and error:
* $2.9^{2.9} \approx 21.8$
* $3.2^{3.2} \approx 41.3$
* $3.3^{3.3} \approx 50.5$
So, **$u \approx 3.28$**.
...
$$\log_{10}(x) \approx 3.28$$
Convert to exponential form:
$$x = 10^{3.28}$$
$$\mathbf{x \approx 1905.46}$$

---

### **Alternative Interpretation**
If the problem was actually **$(\log_x(10)) = \dots$** or if the base was different, the steps would change. But for the equation as written:

> **Key Rule Used:** Substitution. 
> By replacing the repeating log term with a single variable ($u$), you turn a confusing log problem into an exponential growth problem.